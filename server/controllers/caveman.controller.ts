...
const getWorkouts = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const workouts = await db.User.findById(req.params.id).select('workouts');
...
const postWorkout = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const workoutToPost = await db.User.findByIdAndUpdate(id, {
      $push: {
        workouts: {
          title,
          date,
...
const addInfo = async (req: Request, res: Response) => {
  try {
    ...
    const workoutToUpdate = await db.User.findOneAndUpdate(
      {
        'workouts._id': id,
      },
      {
...
const getWorkoutInfo = async (req: Request, res: Response) => {
  try {
    const workoutInfoCards = await db.Exercises.find();
...
export default { getUser, getWorkouts, postWorkout, addInfo, getWorkoutInfo };
